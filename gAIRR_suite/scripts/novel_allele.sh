...
core_allele="${script_dir}/../material/ori_material/${allele_name}_alleles_parsed.fasta"
...
samtools view -b -h ${outer_dir}bwa_read_to_allele.sam > ${outer_dir}bwa_read_to_allele.bam
if [ $? -ne 0 ]; then
    echo "Error: samtools view failed"
fi

samtools view -F 4 -h -b ${outer_dir}bwa_read_to_allele.bam > ${outer_dir}filtered_bwa_read_to_allele.bam
if [ $? -ne 0 ]; then
    echo "Error: samtools view -F 4 failed"
fi
rm ${outer_dir}bwa_read_to_allele.bam

samtools view -h ${outer_dir}filtered_bwa_read_to_allele.bam > ${outer_dir}bwa_read_to_allele.sam
if [ $? -ne 0 ]; then
    echo "Error: samtools view -h failed"
fi
rm ${outer_dir}filtered_bwa_read_to_allele.bam

# start analysis
echo "[gAIRRCall: gAIRR-wgs] [NOVEL ALLELE] Finding novel alleles..."
...
# filter out the novel alleles for flanking
if [ -e ${outer_dir}corrected_alleles_raw.fasta ]
then
    echo "[gAIRRCall: gAIRR-wgs] [NOVEL ALLELE] Parsing novel alleles..."
    python3 ${script_dir}/filter_out_novel_for_flanking.py -fcafil ${outer_dir}corrected_alleles_raw.fasta \
                                                        -fdbfnk ${allele_path} \
                                                        -fdbraw ${core_allele}
fi
...
then
    ...
    echo "[gAIRRCall: gAIRR-wgs] [NOVEL ALLELE] Product refinement..."
    ...
    echo "[gAIRRCall: gAIRR-wgs] [NOVEL ALLELE] Add novel allele to target alleles..."
    ...
    echo "[gAIRRCall: gAIRR-wgs] [NOVEL ALLELE] There are no novel alleles..."
    cp ${allele_path} ${outer_dir}/${allele_name}_with_novel.fasta
fi

echo "[gAIRRCall: gAIRR-wgs] [NOVEL ALLELE] Finished!"